...
def CheckSignature(q, p, g, alpha, beta):
    message = DS.random_string(random.randint(32, 512)).encode(
        'UTF-8')  # generate a random string for message and convert it to bytes
...
if ReturnCode == 0:
    ...
    print("q is not prime"); sys.exit()
elif ReturnCode == -2:
    print("p is not prime"); sys.exit()
elif ReturnCode == -3:
    print("q does not divide p"); sys.exit()
elif ReturnCode == -4:
    print("g is not a generator"); sys.exit()
elif ReturnCode == -5:
    print("p is not 2048 bit"); sys.exit()
elif ReturnCode == -6:
    print("q is not 224 bit"); sys.exit()
...
if ReturnCode == 0:
    ...
    print("Public/private key pair: Failed!"); sys.exit()
...
if ReturnCode == 0:
    ...
    print("Signature generation: Failed!"); sys.exit()

# Test 4: Verifying the signatures in "TestSet.txt"
if (CheckTestSignatures() == 0):
    print("Sample signatures test: Passed!")
else:
    print("Sample signatures test: Failed!"); sys.exit()

# Test 5: generating a single random transaction, sign and verify it
if (CheckTransaction(q, p, g) == 0):
    print("Transaction signature verifies: Passed!")
else:
    print("Transaction signature DOES NOT verify: Failed!"); sys.exit()

# Test 6: Verifying the signatures of transactions in "transactions.txt"
result = CheckBlockofTransactions()
if -1 not in result:
    print("All transactions verified successfully: Passed!")
else:
